 ...
 *
 * In addition to sending a 403 (SC_FORBIDDEN) HTTP error code, it will remove the invalid CSRF
 * cookie from the browser side when a CsrfException occurs. In this way the browser side
 * application, e.g. JavaScript code, can distinguish the CsrfException from other
 * AccessDeniedExceptions and perform more specific operations. For instance, send a GET HTTP method
 * to obtain a new CSRF token.
 *
